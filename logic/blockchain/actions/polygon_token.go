...
func PushPolygonTokenBalance(target common.Address, requestId [32]byte, balance uint64) (string, error) {
	polygonTokenContract, err := polygon_token.NewPolygonToken(target, eth.DefaultClient.Client())
	if err != nil {
		return "", err
	}

	opts, err := eth.DefaultClient.GetDefaultTransactionOptions(context.Background(), config.Get().Wallet.PrivateKey)
	transaction, err := polygonTokenContract.HandleCallback(opts, requestId, new(big.Int).SetUint64(balance))
	if err != nil {
		return "", err
	}
